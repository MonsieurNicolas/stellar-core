...
TEST_CASE("liabilities", "[ledger][liabilities]")
{
    ...
    SECTION("add account selling liabilities")
    {
        auto addSellingLiabilities =
            [&](uint32_t initNumSubEntries, int64_t initBalance,
                ...
                bool res = stellar::addSellingLiabilities(header, acc, deltaLiabilities);
        ...
        for_versions_from(10, *app, [&] {
            ...
            SECTION("below reserve")
            {
                ...
                REQUIRE(
                    addSellingLiabilities(0, lm.getLastMinBalance(0) - 1, 0, 0));

                // Cannot increase when account is below reserve
                REQUIRE(
                    !addSellingLiabilities(0, lm.getLastMinBalance(0) - 1, 0, 1));
            ...
            SECTION("cannot make liabilities negative")
            {
                ...
                REQUIRE(addSellingLiabilities(0, lm.getLastMinBalance(0), 0, 0));
                REQUIRE(addSellingLiabilitiesUninitialized(
                    0, lm.getLastMinBalance(0), 0));
                REQUIRE(!addSellingLiabilities(0, lm.getLastMinBalance(0), 0, -1));
                REQUIRE(!addSellingLiabilitiesUninitialized(
                    0, lm.getLastMinBalance(0), -1));
                ...
                REQUIRE(
                    addSellingLiabilities(0, lm.getLastMinBalance(0) + 1, 1, -1));
                REQUIRE(
                    !addSellingLiabilities(0, lm.getLastMinBalance(0) + 1, 1, -2));

                // Initial liabilities and below maximum
                REQUIRE(
                    addSellingLiabilities(0, lm.getLastMinBalance(0) + 2, 1, -1));
                REQUIRE(
                    !addSellingLiabilities(0, lm.getLastMinBalance(0) + 2, 1, -2));
            }
            ...
            SECTION("cannot increase liabilities above balance minus reserve")
            {
                ...
                REQUIRE(addSellingLiabilitiesUninitialized(
                    0, lm.getLastMinBalance(0) + 1, 1));
                REQUIRE(
                    !addSellingLiabilities(0, lm.getLastMinBalance(0) + 1, 0, 2));
                REQUIRE(!addSellingLiabilitiesUninitialized(
                    0, lm.getLastMinBalance(0) + 1, 2));

                // Initial liabilities and at maximum
                REQUIRE(
                    addSellingLiabilities(0, lm.getLastMinBalance(0) + 1, 1, 0));
                REQUIRE(
                    !addSellingLiabilities(0, lm.getLastMinBalance(0) + 1, 1, 1));
    ...
    SECTION("add account buying liabilities")
    {
        auto addBuyingLiabilities = [&](uint32_t initNumSubEntries,
            ...
            auto header = ls.loadHeader();
            auto acc = ls.create(le);
            bool res = stellar::addBuyingLiabilities(header, acc, deltaLiabilities);
            REQUIRE(acc.current().data.account().balance == initBalance);
            REQUIRE(getSellingLiabilities(header, acc) == initSellingLiabilities);
            if (res)
            {
        ...
        for_versions_from(10, *app, [&] {
            ...
            SECTION("below reserve")
            {
                ...
                // Can leave unchanged when account is below reserve
                REQUIRE(addBuyingLiabilities(0, lm.getLastMinBalance(0) - 1, 0, 0));
                REQUIRE(addBuyingLiabilities(0, lm.getLastMinBalance(0) - 1, 1, 0));

                // Can increase when account is below reserve
                REQUIRE(addBuyingLiabilities(0, lm.getLastMinBalance(0) - 1, 0, 1));
                REQUIRE(addBuyingLiabilities(0, lm.getLastMinBalance(0) - 1, 1, 1));
            }
            ...
            SECTION("cannot make liabilities negative")
            {
                ...
                REQUIRE(addBuyingLiabilitiesUninitialized(
                    0, lm.getLastMinBalance(0), 0));
                REQUIRE(!addBuyingLiabilities(0, lm.getLastMinBalance(0), 0, -1));
                REQUIRE(!addBuyingLiabilitiesUninitialized(
                    0, lm.getLastMinBalance(0), -1));

                // Initial liabilities
                REQUIRE(addBuyingLiabilities(0, lm.getLastMinBalance(0), 1, -1));
                REQUIRE(!addBuyingLiabilities(0, lm.getLastMinBalance(0), 1, -2));
            }

            SECTION("cannot increase liabilities above INT64_MAX minus balance")
            {
                // No initial liabilities, account below reserve
                REQUIRE(addBuyingLiabilities(0, lm.getLastMinBalance(0) - 1, 0,
                                             INT64_MAX -
                                                 (lm.getLastMinBalance(0) - 1)));
                REQUIRE(addBuyingLiabilitiesUninitialized(
                    0, lm.getLastMinBalance(0) - 1,
                    INT64_MAX - (lm.getLastMinBalance(0) - 1)));
                REQUIRE(!addBuyingLiabilities(0, lm.getLastMinBalance(0) - 1, 0,
                                              INT64_MAX -
                                                  (lm.getLastMinBalance(0) - 2)));
                REQUIRE(!addBuyingLiabilitiesUninitialized(
                    0, lm.getLastMinBalance(0) - 1,
                ...
                REQUIRE(addBuyingLiabilities(0, lm.getLastMinBalance(0), 0,
                                             INT64_MAX - lm.getLastMinBalance(0)));
                REQUIRE(addBuyingLiabilitiesUninitialized(
                    0, lm.getLastMinBalance(0), INT64_MAX - lm.getLastMinBalance(0)));
                REQUIRE(!addBuyingLiabilities(0, lm.getLastMinBalance(0), 0,
                                              INT64_MAX -
                                                  (lm.getLastMinBalance(0) - 1)));
                REQUIRE(!addBuyingLiabilitiesUninitialized(
                ...
                REQUIRE(addBuyingLiabilities(0, lm.getLastMinBalance(0), 1,
                                             INT64_MAX -
                                                 (lm.getLastMinBalance(0) + 1)));
                REQUIRE(!addBuyingLiabilities(0, lm.getLastMinBalance(0), 1,
                                              INT64_MAX - lm.getLastMinBalance(0)));
            ...
            SECTION("limiting values")
            {
                ...
                REQUIRE(!addBuyingLiabilities(0, lm.getLastMinBalance(0),
                                              INT64_MAX - lm.getLastMinBalance(0),
                                              1));
                REQUIRE(addBuyingLiabilities(
                    0, lm.getLastMinBalance(0), INT64_MAX - lm.getLastMinBalance(0) - 1,
                    1));
    ...
    SECTION("add trustline selling liabilities")
    {
        auto addSellingLiabilities = [&](int64_t initLimit, int64_t initBalance,
            ...
            bool res = stellar::addSellingLiabilities(header, trust, deltaLiabilities);
            REQUIRE(trust.current().data.trustLine().balance == initBalance);
            REQUIRE(getBuyingLiabilities(header, trust) == initBuyingLiabilities);
            if (res)
            {
    ...
    SECTION("add trustline buying liabilities")
    {
        auto addBuyingLiabilities = [&](int64_t initLimit, int64_t initBalance,
            ...
            auto header = ls.loadHeader();
            auto trust = ls.create(le);
            bool res = stellar::addBuyingLiabilities(header, trust, deltaLiabilities);
            REQUIRE(trust.current().data.trustLine().balance == initBalance);
            REQUIRE(getSellingLiabilities(header, trust) == initSellingLiabilities);
            if (res)
            {
...
TEST_CASE("balance with liabilities", "[ledger][liabilities]")
{
    ...
    SECTION("account add balance")
    {
        auto addBalance = [&](uint32_t initNumSubEntries, int64_t initBalance,
            ...
            REQUIRE(getSellingLiabilities(header, acc) == initLiabilities.selling);
            REQUIRE(getBuyingLiabilities(header, acc) == initLiabilities.buying);
            if (res)
            {
        ...
        for_versions_from(10, *app, [&] {
                    ...
                    "liabilities")
            {
                ...
                REQUIRE(
                    addBalance(0, lm.getLastMinBalance(0), Liabilities{0, 0}, 0));
                REQUIRE(
                    !addBalance(0, lm.getLastMinBalance(0), Liabilities{0, 0}, -1));
    ...
    SECTION("trustline add balance")
    {
        auto addBalance = [&](int64_t initLimit, int64_t initBalance,
            ...
            REQUIRE(getSellingLiabilities(header, trust) == initLiabilities.selling);
            REQUIRE(getBuyingLiabilities(header, trust) == initLiabilities.buying);
            if (res)
            {
...
TEST_CASE("available balance and limit", "[ledger][liabilities]")
{
    ...
    SECTION("trustline available limit")
    {
        auto checkAvailableLimit = [&](int64_t initLimit, int64_t initBalance,
            ...
            if (availableLimit < INT64_MAX)
            {
                REQUIRE(!stellar::addBalance(header, trust, availableLimit + 1));
            }
