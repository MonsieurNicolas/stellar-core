              ...
              std::map<LedgerKey, LedgerStateDelta::EntryDelta> const& expected)
{
    ...
    while (expectedIter != expected.end() && iter != delta.entry.end())
    {
        ...
        REQUIRE((bool)expectedIter->second.current == (bool)iter->second.current);
        if (expectedIter->second.current)
        {
            REQUIRE(*expectedIter->second.current == *iter->second.current);
        }
        REQUIRE((bool)expectedIter->second.previous == (bool)iter->second.previous);
...
TEST_CASE("LedgerState commit into LedgerState", "[ledgerstate]")
{
    ...
    SECTION("one entry")
    {
        SECTION("created in child")
        {
            ...
            validate(ls1, {{key, {std::make_shared<LedgerEntry const>(le1), nullptr}}});
        }
        ...
        SECTION("modified in child")
        {
            ...
            validate(ls1, {{key, {std::make_shared<LedgerEntry const>(le2), nullptr}}});
        }
...
TEST_CASE("LedgerState round trip", "[ledgerstate]")
{
    ...
    auto generateNew = [] (AbstractLedgerState& ls, std::map<LedgerKey, LedgerEntry>& entries) {
    ...
    auto generateModify = [&gen] (AbstractLedgerState& ls, std::map<LedgerKey, LedgerEntry>& entries) {
        ...
        while (modifyBatch.size() < MODIFY_ENTRIES)
        {
            ...
            modifyBatch[iter->first] = generateLedgerEntryWithSameKey(iter->second);
        }
    ...
    auto generateErase = [&gen] (AbstractLedgerState& ls, std::map<LedgerKey, LedgerEntry>& entries,
...
TEST_CASE("LedgerState create", "[ledgerstate]")
{
    ...
    SECTION("when key exists in grandparent, erased in parent")
    {
        ...
        validate(ls3, {{key, {std::make_shared<LedgerEntry const>(le), nullptr}}});
    }
}
...
TEST_CASE("LedgerState erase", "[ledgerstate]")
{
    ...
    SECTION("when key exists in parent")
    {
        ...
        LedgerState ls2(ls1);
        REQUIRE_NOTHROW(ls2.erase(key));
        validate(ls2, {{key, {nullptr, std::make_shared<LedgerEntry const>(le)}}});
    }
    ...
    std::vector<std::map<AccountID, std::pair<AccountID, int64_t>>>::const_iterator const& end)
{
    ...
    if (++begin != end)
    {
        testInflationWinners(ls, maxWinners, minBalance, expected,
                             begin, end);
    }
    else
    {
        ...
        {
            REQUIRE(*expectedIter == std::make_tuple(iter->accountID, iter->votes));
            ++expectedIter;
            ++iter;
        }
        REQUIRE(expectedIter == expected.end());
...
TEST_CASE("LedgerState queryInflationWinners", "[ledgerstate]")
{
    ...
    SECTION("empty parent")
    {
        ...
        SECTION("two voters")
        {
            SECTION("max one winner")
            {
                ...
                {
                    testInflationWinners(1, QUERY_VOTE_MINIMUM, {{a3, 2*QUERY_VOTE_MINIMUM+10}},
                        {{{a1, {a3, QUERY_VOTE_MINIMUM+3}},
                          {a2, {a3, QUERY_VOTE_MINIMUM+7}}}});

                    SECTION("with total near min votes boundary")
                    {
                        testInflationWinners(1, 2*QUERY_VOTE_MINIMUM+10, {{a3, 2*QUERY_VOTE_MINIMUM+10}},
                            {{{a1, {a3, QUERY_VOTE_MINIMUM+3}},
                              {a2, {a3, QUERY_VOTE_MINIMUM+7}}}});
                ...
                {
                    testInflationWinners(1, QUERY_VOTE_MINIMUM, {{a4, QUERY_VOTE_MINIMUM+7}},
                        {{{a1, {a3, QUERY_VOTE_MINIMUM+3}},
            ...
            SECTION("max two winners")
            {
                SECTION("different inflation destinations")
                {
                    testInflationWinners(2, QUERY_VOTE_MINIMUM,
                        inflationSort({{a3, QUERY_VOTE_MINIMUM+3}, {a4, QUERY_VOTE_MINIMUM+7}}),
                        {{{a1, {a3, QUERY_VOTE_MINIMUM+3}},
                          {a2, {a4, QUERY_VOTE_MINIMUM+7}}}});
                    testInflationWinners(2, QUERY_VOTE_MINIMUM+5, {{a4, QUERY_VOTE_MINIMUM+7}},
                        {{{a1, {a3, QUERY_VOTE_MINIMUM+3}},
                          {a2, {a4, QUERY_VOTE_MINIMUM+7}}}});
                }
            }
    ...
    SECTION("one voter in parent")
    {
        ...
        {
            testInflationWinners(2, QUERY_VOTE_MINIMUM, {{a3, QUERY_VOTE_MINIMUM}},
                {{{a1, {a2, QUERY_VOTE_MINIMUM}}},
            ...
            SECTION("max one winner")
            {
                ...
                SECTION("different inflation destinations")
                {
                    testInflationWinners(1, QUERY_VOTE_MINIMUM, {{a4, QUERY_VOTE_MINIMUM+7}},
                        {{{a1, {a3, QUERY_VOTE_MINIMUM+3}}},
                         {{a2, {a4, QUERY_VOTE_MINIMUM+7}}}});
                    testInflationWinners(1, QUERY_VOTE_MINIMUM, {{a3, QUERY_VOTE_MINIMUM+7}},
            ...
            SECTION("max two winners")
            {
                SECTION("different inflation destinations")
                {
                    ...
                    testInflationWinners(2, QUERY_VOTE_MINIMUM+5, {{a4, QUERY_VOTE_MINIMUM+7}},
                        {{{a1, {a3, QUERY_VOTE_MINIMUM+3}}},
                         {{a2, {a4, QUERY_VOTE_MINIMUM+7}}}});
                    testInflationWinners(2, QUERY_VOTE_MINIMUM+5, {{a3, QUERY_VOTE_MINIMUM+7}},
                        {{{a1, {a3, QUERY_VOTE_MINIMUM+7}}},
                         {{a2, {a4, QUERY_VOTE_MINIMUM+3}}}});
                }
            }
...
TEST_CASE("LedgerState load", "[ledgerstate]")
{
    ...
    SECTION("when key exists in parent")
    {
        ...
        LedgerState ls2(ls1);
        REQUIRE(ls2.load(key));
        validate(ls2, {{key, {std::make_shared<LedgerEntry const>(le),
...
static void testAllOffers(
    ...
    std::vector<std::map<std::pair<AccountID, uint64_t>,
                         std::tuple<Asset, Asset, int64_t>>>::const_iterator begin,
    std::vector<std::map<std::pair<AccountID, uint64_t>,
    ...
    else
    {
        ...
        while (expectedIter != expected.end() && iter != offers.end())
        {
                   ...
                   innerIter != inner.end())
            {
                ...
                REQUIRE(*expectedInnerIter == std::make_tuple(oe.offerID, oe.buying, oe.selling, oe.amount));
                ++expectedInnerIter;
                ++innerIter;
            }
...
TEST_CASE("LedgerState loadAllOffers", "[ledgerstate]")
{
    ...
    SECTION("empty parent")
    {
        ...
        SECTION("two offers")
        {
            ...
            {
                testAllOffers(
                    {{a1, {{1, buying, selling, 1}}}, {a2, {{2, buying, selling, 1}}}},
                    {{{{a1, 1}, {buying, selling, 1}},
...
static void testBestOffer(
    ...
    std::vector<std::map<std::pair<AccountID, uint64_t>,
                         std::tuple<Asset, Asset, Price, int64_t>>>::const_iterator begin,
    std::vector<std::map<std::pair<AccountID, uint64_t>,
                         std::tuple<Asset, Asset, Price, int64_t>>>::const_iterator const& end)
{
    REQUIRE(begin != end);
    LedgerState ls(lsParent);
    applyLedgerStateUpdates(ls, *begin);
    ...
    else
    {
        ...
        if (offer)
        {
            ...
            REQUIRE(expected.front() == std::make_tuple(oe.offerID, oe.buying, oe.selling, oe.price, oe.amount));
        }
        else
        {
...
static void testBestOffer(
    ...
    std::vector<std::tuple<uint64_t, Asset, Asset, Price, int64_t>> const& expected,
    std::vector<std::map<std::pair<AccountID, uint64_t>,
                         std::tuple<Asset, Asset, Price, int64_t>>> updates)
{
    REQUIRE(!updates.empty());
    ...
    auto testAtRoot = [&] (Application& app) {
        ...
        testBestOffer(app.getLedgerStateRoot(), buying, selling,
                      expected, ++updates.cbegin(), updates.cend());
    };
...
TEST_CASE("LedgerState loadBestOffer", "[ledgerstate]")
{
    ...
    SECTION("empty parent")
    {
        ...
        SECTION("two offers")
        {
            SECTION("same assets")
            {
                SECTION("same price")
                {
                    testBestOffer(buying, selling, {{1, buying, selling, Price{1, 1}, 1}},
                ...
                {
                    testBestOffer(buying, selling, {{2, buying, selling, Price{1, 1}, 1}},
    ...
    SECTION("one offer in parent")
    {
        ...
        {
            testBestOffer(buying, selling, {{1, buying, selling, Price{2, 1}, 7}},
                {{{{a1, 1}, {buying, selling, Price{1, 1}, 1}}},
        ...
        SECTION("other offer in child")
        {
            ...
            testBestOffer(buying, selling, {{2, buying, selling, Price{1, 2}, 1}},
                {{{{a1, 1}, {buying, selling, Price{1, 1}, 1}}},
                         ...
                         std::tuple<Asset, Asset, int64_t>>>::const_iterator const& end)
{
    ...
    else
    {
        ...
        while (expectedIter != expected.end() && iter != offers.end())
        {
            ...
            REQUIRE(*expectedIter == std::make_tuple(oe.offerID, oe.buying, oe.selling, oe.amount));
            ++expectedIter;
            ++iter;
        }
        REQUIRE(expectedIter == expected.end());
...
static void testOffersByAccountAndAsset(
    ...
    std::vector<std::map<std::pair<AccountID, uint64_t>,
                         std::tuple<Asset, Asset, int64_t>>> updates)
{
    REQUIRE(!updates.empty());
...
TEST_CASE("LedgerState loadOffersByAccountAndAsset", "[ledgerstate]")
{
    ...
    SECTION("empty parent")
    {
        ...
        {
            testOffersByAccountAndAsset(a1, buying,
                {{1, buying, native, 1}, {2, buying, native, 1}},
    ...
    SECTION("two offers in parent")
    {
        testOffersByAccountAndAsset(a1, buying,
            ...
            {{{{a1, 1}, {buying, native, 1}},
              {{a1, 2}, {buying, native, 1}}}, {}});
        testOffersByAccountAndAsset(a1, buying,
            {{1, buying, native, 1}, {2, native, buying, 1}},
            {{{{a1, 1}, {buying, native, 1}},
              {{a1, 2}, {native, buying, 1}}}, {}});
    }
}
...
TEST_CASE("LedgerStateEntry and LedgerStateHeader move assignment", "[ledgerstate]")
{
    VirtualClock clock;
    auto app = createTestApplication(clock, getTestConfig());
    app->start();
