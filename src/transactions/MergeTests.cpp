...
TEST_CASE("merge", "[tx][merge]")
{
    ...
    {
        auto mergeFrom =
            root.create("merge-from", app->getLedgerManager().getLastMinBalance(0));
        for_all_versions(*app, [&] {
    ...
    {
        auto mergeFrom = root.create(
            "merge-from", app->getLedgerManager().getLastMinBalance(0) + txfee - 1);
        for_all_versions(*app, [&] {
            REQUIRE_THROWS_AS(mergeFrom.merge(root), ex_txINSUFFICIENT_BALANCE);
        });
    }
    ...
    {
        auto mergeFrom = root.create(
            "merge-from", app->getLedgerManager().getLastMinBalance(0) + 2 * txfee);
        closeLedgerOn(*app, 3, 1, 1, 2017);
