                          ...
                          Price const& wheatPrice)
{
    ...
    // compute value based on what the account can receive
    auto sellerMaxSheep =
        sheepLine ? sheepLine.getMaxAmountReceive(header) : 0;
              ...
              Asset const& asset, TrustLineWrapper const& trustLine)
{
    if (asset.type() == ASSET_TYPE_NATIVE)
    {
        // can only send above the minimum balance
        return std::max(
            {getAvailableBalance(header, account), int64_t(0)});
    }

    if (trustLine && trustLine.isAuthorized())
    {
        return std::max(
            {trustLine.getAvailableBalance(header), int64_t(0)});
    }
...
int64_t
canSellAtMost(LedgerStateHeader const& header,
              ConstLedgerStateEntry const& account,
             ...
             Asset const& asset, TrustLineWrapper const& trustLine)
{
    ...
    {
        return std::max(
            {getMaxAmountReceive(header, account), int64_t(0)});
    }
    else
    {
        return trustLine
                   ? std::max({trustLine.getMaxAmountReceive(header),
                               int64_t(0)})
                   : 0;
    }
}
...
LoadBestOfferContextOld::LoadBestOfferContextOld(Database& db, Asset const& selling,
                                           Asset const& buying)
                ...
                int64_t maxSheepSend, int64_t& numSheepSend, int64_t& newAmount)
{
    ...
    auto exchangeResult = header.current().ledgerVersion < 3
        ? exchangeV2(numWheatReceived, price, maxWheatReceived, maxSheepSend)
        : exchangeV3(numWheatReceived, price, maxWheatReceived, maxSheepSend);
           ...
           std::vector<ClaimOfferAtom>& offerTrail)
{
    ...
    if (numSheepSend != 0)
    {
        ...
        {
            auto sheepLineAccountB = stellar::loadTrustLine(lsInner, accountBID, sheep);
    ...
    if (numWheatReceived != 0)
    {
        ...
        {
            auto wheatLineAccountB = stellar::loadTrustLine(lsInner, accountBID, wheat);
              ...
              bool isPathPayment, std::vector<ClaimOfferAtom>& offerTrail)
{
    ...
    int64_t maxWheatSend =
        canSellAtMost(header, accountB, wheat, wheatLineAccountB);
    maxWheatSend =
        std::min({offer.amount, maxWheatSend});
    int64_t maxSheepReceive =
        canBuyAtMost(header, accountB, sheep, sheepLineAccountB);
    auto exchangeResult = exchangeV10(
        offer.price, maxWheatSend, maxWheatReceived,
        maxSheepSend, maxSheepReceive, isPathPayment);
    ...
    std::vector<ClaimOfferAtom>& offerTrail)
{
    ...
    while (needMore)
    {
        ...
        {
            cor = crossOffer(ls, wheatOffer, maxWheatReceive,
                             numWheatReceived, maxSheepSend, numSheepSend,
                             offerTrail);
            needMore = true;
        }
