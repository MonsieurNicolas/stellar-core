...
LedgerStateEntry
loadData(AbstractLedgerState& ls, AccountID const& accountID, std::string const& dataName)
{
    LedgerKey key(DATA);
...
TrustLineWrapper
loadTrustLine(AbstractLedgerState& ls, AccountID const& accountID, Asset const& asset)
{
    return TrustLineWrapper(ls, accountID, asset);
}

ConstTrustLineWrapper
loadTrustLineWithoutRecord(AbstractLedgerState& ls, AccountID const& accountID, Asset const& asset)
{
                                  ...
                                  LedgerEntry const& entry)
{
    if (header.current().ledgerVersion < 10)
    {
        throw std::runtime_error("Offer liabilities calculated before version 10");
    }
...
{
    auto& acc = entry.current().data.account();
    std::sort(acc.signers.begin(), acc.signers.end(),
