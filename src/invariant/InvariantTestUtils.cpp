               ...
               std::nullptr_t previous)
{
    UpdateList updates;
    std::transform(
        current.begin(), current.end(), std::back_inserter(updates),
...
UpdateList
makeUpdateList(std::nullptr_t current,
               std::vector<LedgerEntry> const& previous)
{
    UpdateList updates;
    std::transform(
        previous.begin(), previous.end(), std::back_inserter(updates),
        [](LedgerEntry const& prev) {
            auto prevPtr = std::make_shared<LedgerEntry>(prev);
